...
function pProjP()
    sim = qDSim(dt = 10.0^(-4) , track = false)
    simX = sim[1]
    simY = sim[2]
    simT = sim[3]
...
function newton_method(f::Function, f_p::Function, x0, tol=1e-12)
    x = x0
    prev_x = x + 2*tol  # Initialize prev_x to a value different from x
...
function secent_method(f::Function, x0, tol=1e-12)
    c_x = x0
    p_x = x0+2*tol
    while abs(c_x-p_x) > tol
        n_x = c_x - f(c_x)*(c_x-p_x)/(f(c_x)-f(p_x))
        p_x = c_x
        c_x = n_x

        println("Secent extimate: ", c_x)
    end
...
function errorRV1(vi , vf , N)
    ...
    for vn in V
        ...
        instInputs(velocity = vn , theta = .95)
...
function errorRV2(vi , vf , N)
    ...
    for vn in V
        ...
        instInputs(theta = .49 , velocity = vn)
...
function errorRV3(vi , vf , N)
    ...
    for vn in V
        ...
        instInputs(theta = .05 , velocity = vn)
        ...
        apT = newton_method(ProjApY , ProjApDY , simTime , 1e-6)
        apR = x3(apT)
...
function errorRTh1(θi , θf , N)
    ...
    for θn in T
        ...
        instInputs(velocity = 25.0 , theta = θn)
...
function errorRTh2(θi , θf , N)
    ...
    for θn in T
        ...
        instInputs(theta = θn , velocity = 25.0)
...
function errorRTh3(θi , θf , N)
    ...
    for θn in T
        ...
        apT = newton_method(ProjApY , ProjApDY , simTime , 1e-5)
        apR = x3(apT)
...
function errorTiV1(vi , vf , N)
    ...
    for vn in V
        ...
        println("Sim Time: ",simT)

        apT = newton_method(ProjApY , ProjApDY , simT)
        println("Aprox Time: ",apT)
        error = abs(simT-apT)/simT
        println("error: " , error)
        push!(errorData , error)
    end
...
function errorTiV2(vi , vf , N)
    ...
    for vn in V
        ...
        instInputs(theta = .475 , velocity = vn)
...
function errorTiV3(vi , vf , N)
    ...
    for vn in V
        ...
        apT = newton_method(ProjApY , ProjApDY , simT , 1e-6)
...
function errorTiTh3(θi , θf , N)
    ...
    for θn in T
        ...
        apT = newton_method(ProjApY , ProjApDY , simT , 1e-5)
...
function errorTpV1(vi , vf , N)
    ...
    for vn in V
        ...
        apTp = secent_method(ProjApDY , simTp)

        error = abs(simTp-apTp)/simTp
        println("Ap Tp: ",apTp)
        println("error: ",error)
        push!(errorData , error)
    end
